    ...
    def stat(node, params):
        ...
        try:
            if params["expr"] == '*':
                for expr in server_dict:
###
#		    print '@ RPC.stat(node,params[expr]=*): {for expr in server_dict}: RemoteProcedureCall._stat_server(node:%s, expr:%s)' % (node.node_id,expr)
###
    ...
    @staticmethod
    def seed(node, params, state=False):
        ...
        try:
            for expr in params["expr"].split("|"):
                RemoteProcedureCall._seed_server(node, expr, transaction, state)
        except lib.RemoteProcedureCallError:
            raise
        return True
