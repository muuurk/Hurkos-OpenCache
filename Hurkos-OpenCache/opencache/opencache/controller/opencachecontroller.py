...
class Controller():
    ...
    def _create_config_defaults(self):
        ...
        config['verbosity'] = '3'
	config['openflow_switch_dpid'] = '00:00:00:00:00:00:00:01'
	config['ovs_dpid'] = '00:00:00:00:00:00:00:02'
	config['ofswitch_outport_to_ovs'] = '1'
	config['ofswitch_outport_to_server'] = '1'
	config['ofswitch_inport_from_ovs'] = '1'
	config['ofswitch_outport_to_ocn'] = '1'
	config['ofswitch_outport_to_client'] = '1'
        return config
    ...
    def stat(self, rpc):
        ...
        try:
            ...
            if rpc.node_id == '*':
                for node in self._state.list_nodes():
                    total_result = self._stat_node(rpc.expr, node, total_result)	# innen hívódik tovább, ha minden node-ra kell
            elif "|" in rpc.node_id:
                for node in rpc.node_id.split("|"):
                    total_result = self._stat_node(rpc.expr, node, total_result)
            else:
                total_result = self._stat_node(rpc.expr, rpc.node_id, total_result)	# innen hívódik tovább: _stat_node()
...
# elif a *-ra egy módosított .get_stat() fv. (ami tömböt ad vissza) majd else-szel az eredetire
	    elif expr == '*':
		stats = self._state.get_stat_asterix(expr, node)
	        for stat in stats:
			total_result = self._add_to_total(stat, total_result)
            else: 
		stat = self._state.get_stat(expr, node)
...
### 		IDE érkezik be a stat kérés, de az csak a refresh kérésre fut le (30sec-re vagy amikor kérem)
    ...
    def _handle_redir_message(self, notification):
        ...
        if notification['params']['action'] == 'add':
            try:
                self._controller._request_handling.add_redirect(str(notification['params']['expr']), str(notification['params']['host']), str(notification['params']['port']), str(self._controller.config['openflow_host']), str(self._controller.config['openflow_port']), str(self._controller.config['ovs_dpid']), str(self._controller.config['openflow_switch_dpid']), str(self._controller.config['ofswitch_outport_to_ovs']), str(self._controller.config['ofswitch_outport_to_server']), str(self._controller.config['ofswitch_inport_from_ovs']), str(self._controller.config['ofswitch_outport_to_ocn']), str(self._controller.config['ofswitch_outport_to_client']))
            except KeyError:
            ...
            self._controller._request_handling.remove_redirect(str(notification['params']['expr']), str(notification['params']['host']), str(notification['params']['port']), str(self._controller.config['openflow_host']), str(self._controller.config['openflow_port']), str(self._controller.config['ovs_dpid']), str(self._controller.config['openflow_switch_dpid']), str(self._controller.config['ofswitch_outport_to_ovs']), str(self._controller.config['ofswitch_outport_to_server']), str(self._controller.config['ofswitch_inport_from_ovs']), str(self._controller.config['ofswitch_outport_to_ocn']), str(self._controller.config['ofswitch_outport_to_client']))
