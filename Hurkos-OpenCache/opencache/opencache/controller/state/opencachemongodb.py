...
class State:
    ...
    def get_stat_asterix(self, expr, node_id):
        """Get the stats relating to a specific cache instance. (expr=* case) """
        try:
	    stats = []

	    for exprs_result in self._database.statistics.find({'node_id': int(node_id)}):

		stat = self._Stat()

		stat.node_id = exprs_result['node_id']
		stat.expr = exprs_result['expr']
            	stat.status = exprs_result['status']
            	stat.avg_load = exprs_result['avg_load']
            	stat.cache_miss = exprs_result['cache_miss']
            	stat.cache_miss_size = exprs_result['cache_miss_size']
            	stat.cache_hit = exprs_result['cache_hit']
            	stat.cache_hit_size = exprs_result['cache_hit_size']
            	stat.cache_object = exprs_result['cache_object']
            	stat.cache_object_size = exprs_result['cache_object_size']
...
##
		stat.expr = result['expr']
		stat.node_id = result['node_id']
##
